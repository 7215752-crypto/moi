...
| `/analytics` | Товарная аналитика: продажи блюд из iiko, ABC-анализ, допродажи модификаторами, фудкост, вкладки «По категориям» и «Требует внимания», экспорт CSV |
| `/analytics/dish/[dishId]` | Карточка блюда: разбивка по ресторанам, время отдачи по цехам, модификаторы, фудкост и график его динамики |
| `/analytics/dish/[dishId]/chart` | Техкарта блюда из iiko (открывается из карточки в новой вкладке) |
| `/api/dish-sales` | POST — импорт продаж из iiko OLAP за период в `dish_sales_daily` (owner/accountant/manager) |
| `/api/dish-sales/export` | GET — выгрузка таблицы блюд в CSV с учётом фильтров |
